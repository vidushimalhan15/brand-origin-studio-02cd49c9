  ...
  return (
    <AppLayout>
      <div className="space-y-8">
        <header>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Campaign Setup</h2>
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {pillars.map((p) => (
                  <span
                    key={p}
                    className="group text-xs bg-slate-50 border border-slate-100 pl-3 pr-1.5 py-1 rounded-full text-slate-600 flex items-center gap-1"
                  >
                    {p}
                    <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {events.map((event) => {
              ...
              return (
                <div
                  key={event.name}
                  className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3 hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors"
                >
                  <div className="px-2 py-1 bg-indigo-100 text-indigo-700 text-[10px] font-bold rounded flex flex-col items-center min-w-[36px]">
                    <span>{m}</span>
                    <span>{d}</span>
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {PLATFORMS.map((p) => {
              ...
              return (
                ...
                >
                  <p.icon className={`w-5 h-5 ${active ? p.color : "text-slate-400"}`} />
        ...
        {/* CTA */}
        <button className="w-full bg-indigo-600 p-8 rounded-3xl text-white flex items-center justify-between group hover:bg-indigo-700 transition-all text-left shadow-sm hover:shadow-md">
          <div>
